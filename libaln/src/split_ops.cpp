...
void splitControl(ALN* pALN, double dblLimit)  // routine
{
  ASSERT(pALN);
	...
	// get square errors of pieces on training set and the noise variance estimates from VARfile
...
void splitUpdateValues(ALN * pALN) // routine
{
	...
	// IMPORTANT: THE SAMPLE DOMAINS IN TRfile AND VARfile MUST BE IN 1-TO-1 CORRESPONDENCE
	...
	for (long i = 0; i < nrows; i++) // TRfile and VARfile have to have a 1-to-1 correspondence of samples
	{
		...
		if (LFN_CANSPLIT(pActiveLFN)) // Skip this leaf node if it can't split anyway.
		{
			...
			fromFile = VARfile.GetAt(i, nDimm1, 0);
		...
		if (LFN_CANSPLIT(pNode))
		{
			...
			if (Count >= nDim) // Don't consider splitting if there are too few samples; do nothing.
			{
				...
				if (dblLimit <= 0) // if this is TRUE, we do the F test.
				{
					...
					dblSplitLimit = adblFconstant[dofIndex];
				...
				{
					// The piece has had many samples to adjust itself fit well, but has failed.
					// It needs to split and training must continue.
